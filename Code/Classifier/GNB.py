...
for train, test in skf.split(X,y):
    ...
    print('GNB:acc=%f,precision=%f,npv=%f,sensitivity=%f,specificity=%f,mcc=%f,f1=%f,roc_auc=%f'
...
data_csv.to_csv('GNB.csv')

row=yscore.shape[0]
yscore=yscore[np.array(range(1,row)),:]
yscore = pd.DataFrame(data=yscore)
yscore.to_csv('yscore_GNB.csv')
ytest=ytest[np.array(range(1,row)),:]
ytest = pd.DataFrame(data=ytest)
ytest.to_csv('ytest_GNB.csv')